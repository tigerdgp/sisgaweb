<?php
	 ...
	 *	- v0.1.2: Added BR support (MT - 2015-10-19)
	 *
	 ...
	 * -------------------------------------------------------------
	 */
	function smarty_modifier_phone_format($sPhone, $sCountry = 'BR', $bInternationalFormat = false)
	{
		if (empty($sPhone)
		|| !trim($sPhone)) {
			return $sPhone;
		...
		case 'FR':
			...
			case 'UK':	// United Kingdom
				...
				case 'BR':	// Brazil
				// National format: (00) 1234 5678
				// National format: (00) 9 1234 5678
				// International format: +55 (20) 1234-5678
				// Toll number format: 0800-123-4567
				$iLen = strlen($sPhone);
				...
				switch ($iLen) {
					case 8:
						...
						// Note: International number format cannot
						// be used for UK
						$sFormatted = substr($sFormatted, 0, 2) . ' ' . substr($sFormatted, -4);
						$bInternationalFormat &= $sCountry != 'BR';
						break;
	
					case (10):
						// Full number
						$sFormatted = '(' . substr($sFormatted, 0, 2) . ') ' . substr($sFormatted, 2, 4) . '-' . substr($sFormatted, 6);
						break;
					
					case (11):
						// Full number	
						$sFormatted = '(' . substr($sFormatted, 0, 2) . ') ' . substr($sFormatted, 2, 1) . '-' . substr($sFormatted, 3, 4) . '-' . substr($sFormatted, 7);
						break;
